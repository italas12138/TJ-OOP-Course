...
int squdis(coordinate n1, coordinate n2) { //计算两点之间距离的平方和
...
void draw() {
	...
	if (lastsec % 15 == 0) { //画新秒针
		setcolor(RGB(235, 112, 76));
		setlinestyle(PS_SOLID, 2.0);
		line(320, 240, sec.x, sec.y);
	}
	else SDF_line({ 320, 240 }, sec, 0.8, RGB(235, 112, 76), RGB(255, 235, 205));

	if (t.tm_min == lastmin) {//如果分针不用更新，则再画一次上一个分针与时针【是为了防止因擦去旧秒针而造成其他指针部分缺失】
		if (lastmin % 15 == 0) {
			setcolor(RGB(235, 112, 76));
			setlinestyle(PS_SOLID, 2.8);
			line(320, 240, min.x, min.y);
		}
		else SDF_line({ 320,240 }, min, 1.4, RGB(235, 112, 76), RGB(255, 235, 205));
		
		SDF_line({ 320,240 }, hour, 1.6, RGB(235, 112, 76), RGB(255, 235, 205));
	...
	SDF_line({ 320,240 }, min, 1.4, RGB(235, 112, 76), RGB(255, 235, 205));//画新分针
	...
	SDF_line({ 320,240 }, hour, 1.6, RGB(235, 112, 76), RGB(255, 235, 205));//画新时针
}
int main()
{
	//初始化表盘
